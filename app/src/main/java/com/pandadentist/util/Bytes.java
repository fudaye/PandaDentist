...
public class Bytes {
    ...
    private static final char[] PRINTCHAR = new char[126 - 32];
    ...
    public static byte[] getSHA512(byte[] source) {
        MessageDigest md = getMessageDigest("SHA-512");
        return md.digest(source);
    }
    ...
    public static String bytes2hexString(final String str) {
        ...
        for(int i = 0; i < size; i+=16)
        {
            ...
            for(int j=0; j<16; j++)
            {
                if((j+i) < size) {
                    ...
                    //snprintf(tmp, sizeof(tmp), "%02x",(unsigned char)val);
                    tmp = "";
    ...
    public static int crc16(byte[] bytesArr, int len) {
        int i, j;
        int c, crc = 0xFFFF;
        for (i = 0; i < len; i++) {
            c = bytesArr[i] & 0x00FF;
    ...
    private static MessageDigest getMessageDigest(String type) {
        ...
        if (ret == null) {
            try {
                ret = MessageDigest.getInstance(type);
                MD.set(ret);
    ...
    public static String randString(int length) {
        ...
        for (int i = 0; i < length; i++)
            sb.append((char)(min + r.nextInt(max - min)));
